...
import requests
from core.gmail_utils import send_email
...
def send_grade_notification_email(student, teacher, subject, classroom, grade_type=None, is_bulk=False):
    """Helper function to send grade notification emails"""
    if not student.user or not student.user.email:
        print(f"Không thể gửi email: Sinh viên {student.name} không có email")
        return
        
    try:
        # Thêm thông tin loại điểm vào tiêu đề và nội dung email
        grade_type_name = grade_type.name if grade_type else "điểm"
        subject_text = f"[THÔNG BÁO] ĐIỂM {grade_type_name.upper()} MÔN {subject.name} ĐÃ ĐƯỢC CẬP NHẬT"
        body = f"""
Xin chào {student.name},

Giáo viên {teacher.user.get_full_name()} đã cập nhật điểm {grade_type_name} cho bạn trong môn {subject.name} (Lớp: {classroom.name}).

Vui lòng đăng nhập vào hệ thống Score Input để xem chi tiết.

Trân trọng,
Score Input System
"""
        print(f"Đã gửi email thành công cho {student.name} ({student.user.email})")
        send_email(student.user.email, subject_text, body)
    except Exception as e:
        print(f"Lỗi gửi email cho {student.name} ({student.user.email}): {e}")
        
def send_grade_notification_email_upload(student, teacher, subject, classroom, grade_type=None, is_bulk=False):
    """Helper function to send grade notification emails"""
    if not student.user or not student.user.email:
        print(f"Không thể gửi email: Sinh viên {student.name} không có email")
        return
    ...
    try:
        # Thêm thông tin loại điểm vào tiêu đề và nội dung email
        grade_type_name = grade_type.name if grade_type else "điểm"
        subject_text = f"[THÔNG BÁO] ĐIỂM MÔN {subject.name} ĐÃ ĐƯỢC CẬP NHẬT"
        body = f"""
Xin chào {student.name},

Giáo viên {teacher.user.get_full_name()} đã cập nhật điểm cho bạn trong môn {subject.name} (Lớp: {classroom.name}).
        ...
        try:
            ...
            if grade_id:
                ...
                if str(grade.value) != str(new_value):
                    ...
                    # Thêm logging trước khi gửi email
                    print(f"Đang gửi email cho sinh viên {student.name} với điểm {grade_type.name}")
                    
                    send_grade_notification_email(
                        student=student,
                        teacher=request.user.teacher,
                        subject=grade.teacher_assignment.subject,
                        classroom=grade.teacher_assignment.classroom,
        ...
        if form.is_valid():
            ...
            for student in students:
                ...
                if not created:
                    ...
                    # Send email notification for each student's grade update
                    send_grade_notification_email(
                        student=student,
                        teacher=request.user.teacher,
                                ...
                                if cell_value is not None:
                                    try:
                                        ...
                                        if 0 <= grade_value <= 10:
                                            ...
                                            if not created:
                                                # Only update and send email if grade value changed
                                                ...
                                                # For new grades, record history and send email
                                                GradeHistory.objects.create(
                                                    grade=grade,
                                                    new_value=grade_value,
                                                    modified_by=request.user,
                                            ...
                                            if should_send_email:
                                                try:
                                                    # Send email notification for each grade update
                                                    send_grade_notification_email_upload(
                                                        student=student,
                                                        teacher=request.user.teacher,
                                                        subject=assignment.subject,
                                                        classroom=assignment.classroom,
                                                    ...
                                                    print(f"Error sending email to {student.name} ({student.user.email if student.user else 'no email'}): {str(e)}")
